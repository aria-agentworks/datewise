...
export default function Home() {
  ...
  }, [isSignedIn, isLoaded, currentView, setView])
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${
        active ? 'bg-rose-50 text-rose-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
      }`}
    >
      {icon}
      {label}
...
function MobileNavButton({ icon, label, active, onClick, highlight = false }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void; highlight?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl text-xs font-medium transition-all ${
        highlight
          ...
          : active ? 'bg-rose-50 text-rose-600' : 'text-gray-400'
      }`}
    >
      {icon}
      <span>{label}</span>
